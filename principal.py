...
def perform_full_scan(target_input, main_report_file_handle, st_api_key, vt_api_key, vd_api_key, base_directory):
    ...
    if is_ip_address_local: # Input was an IP
        if cloudflare_results_local.get('is_cloudflare'):
            # Input is an IP and it's Cloudflare. Do not scan.
            main_report_file_handle.write(f"La entrada es una IP ({target_ip_for_scan_local}) y está detrás de Cloudflare. Escaneo de puertos omitido para esta IP.\n")
            # ips_to_actually_scan remains empty
        else:
            # Input is an IP and not Cloudflare. Scan it.
            if target_ip_for_scan_local:
                ...
                # This case should ideally not happen if target_ip_for_scan_local is set when is_ip_address_local is true
                main_report_file_handle.write("La entrada es una IP pero target_ip_for_scan_local está vacío. No se puede escanear puertos.\n")
    else: # Input was a Domain
        if cloudflare_results_local.get('is_cloudflare'):
            # Domain resolves to Cloudflare. Scan only origin IPs.
            if consolidated_potential_origin_ips_set_local:
                ips_to_actually_scan.extend(list(consolidated_potential_origin_ips_set_local))
                main_report_file_handle.write(f"Dominio '{dominio_for_report}' resuelve a IP de Cloudflare ({target_ip_for_scan_local}). El escaneo de puertos y ping se realizará sobre las IPs de origen identificadas: {', '.join(ips_to_actually_scan)}\n")
            else:
                main_report_file_handle.write(f"Dominio '{dominio_for_report}' resuelve a IP de Cloudflare ({target_ip_for_scan_local}), y no se encontraron IPs de origen (no Cloudflare). Escaneo de puertos será omitido.\n")
                # ips_to_actually_scan remains empty
        else:
            # Domain does not resolve to Cloudflare. Scan the resolved IP.
            if target_ip_for_scan_local:
                ips_to_actually_scan.append(target_ip_for_scan_local)
                main_report_file_handle.write(f"Cloudflare no detectado para '{dominio_for_report}'. El escaneo de puertos y ping se realizará sobre la IP resuelta: {target_ip_for_scan_local}\n")
            else:
                # This case implies DNS resolution failed earlier, which should be handled before this block.
                # However, as a safeguard:
                main_report_file_handle.write(f"No se pudo determinar una IP para el escaneo de puertos para '{dominio_for_report}' (resolución inicial falló y no está detrás de Cloudflare).\n")
